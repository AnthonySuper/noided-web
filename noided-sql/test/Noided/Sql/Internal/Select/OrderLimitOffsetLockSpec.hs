  ...
  describe "OrderLimitOffsetLock" $ do
    ...
    describe "SelectM" $ do
      ...
      renderGolden "Simple ORDER BY" $
        OrderLimitOffsetLockSelect simpleSelect $ \res ->
          (res, MkOrderLimitOffsetLockCfg Nothing NoFetchFirstClause (OrderQueryBy (asc_ col1)) NoLockingClause)

      renderGolden "ORDER BY, OFFSET, FETCH FIRST" $
        OrderLimitOffsetLockSelect simpleSelect $ \res ->
          (res, MkOrderLimitOffsetLockCfg (Just 10) (FetchFirstClauseOnly 5) (OrderQueryBy (desc_ col1)) NoLockingClause)

      renderGolden "ORDER BY, OFFSET, FETCH FIRST WITH TIES" $
        OrderLimitOffsetLockSelect simpleSelect $ \res ->
          (res, MkOrderLimitOffsetLockCfg (Just 10) (FetchFirstClauseWithTies 5) (OrderQueryBy (desc_ col1)) NoLockingClause)

      renderGolden "Locking FOR UPDATE" $
        OrderLimitOffsetLockSelect simpleSelect $ \res ->
          (res, MkOrderLimitOffsetLockCfg Nothing NoFetchFirstClause NoOrder (LockingClause ForUpdate Wait))

      renderGolden "Locking FOR SHARE SKIP LOCKED" $
        OrderLimitOffsetLockSelect simpleSelect $ \res ->
          (res, MkOrderLimitOffsetLockCfg Nothing NoFetchFirstClause NoOrder (LockingClause ForShare SkipLocked))

    describe "AggregateQuery" $ do
      let simpleAgg :: AggregateQuery (TableAgg (SqlExpr Aggregated))
          simpleAgg = aggregate_ (\r -> TableAgg (agg $ count_ r.t1Id) (agg $ sum_ r.t1Val)) (select_ (Table1 col1 col1))
